...
Andon (schema `andon`): aviso de mantenimiento vencido si km desde la última visita **cerrada** ≥ `t_km` o días ≥ `t_dias` (umbrales por tipo). Sin visita cerrada previa no abre. Máximo un aviso no resuelto por unidad. Unidades inactivas: no avisos nuevos. Enterado (Supervisor) es **in-app** (no promete envío WhatsApp). Outbound: puerto `NotifyPort`, default **noop/log**. Twilio aparcado. Evolution API no está en este PR (adaptador lab de otro agente; WhatsApp no oficial / riesgo de ToS; no producción). Resolver solo con `VisitaCerrada`. Las alertas de stock **no** viven en Andon (ADR-005).
