...
Andon (schema `andon`): aviso de mantenimiento vencido si km desde la última visita **cerrada** ≥ `t_km` o días ≥ `t_dias` (umbrales por tipo). Sin visita cerrada previa no abre. Máximo un aviso no resuelto por unidad. Unidades inactivas: no avisos nuevos. Enterado (Supervisor) es **in-app** (no promete envío WhatsApp). Outbound: puerto `AndonNotifier` / `NotifyPort`, default **noop/log**. Twilio queda aparcado (`ANDON_NOTIFY_PROVIDER=twilio` + credenciales). Resolver solo con `VisitaCerrada`. Las alertas de stock **no** viven en Andon (ADR-005).
