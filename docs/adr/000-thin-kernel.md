...
El kernel no conoce stock, costos ni catálogo de refacciones. Inventario no conoce la entidad Visita: solo IDs opacos (`visitaId`, `tipoVehiculoId`) y el payload `VisitaCerrada.consumos`. Andon consume el mismo envelope e ignora `consumos`. Notifications (schema `notifications`, ADR-006) agrega avisos Andon e Inventario (`StockBajo` / `StockReabastecido`, ADR-007) sin FKs cruzadas.

## Extensiones

- [ADR-008](008-flota-schema.md): rol `LOGISTICA`, `motivoInactivacion` en Unidad, schema `flota` (bitácora de patio). No reescribe este ADR.
